...
export const LabeledInput:React.FC<LabeledInputProps> = (props)=>{
    return(
        <div className="flex flex-col items-center w-full">
            ...
            <input name={props.name} type={"text"} className="border-[1px] w-full rounded-md border-black outline-none focus-within:outline-1 focus-within:outline-red-500 duration-200" id={props.inputId}/>
        </div>
    )
}
...
export const LabeledSelect:React.FC<LabeledSelectProps> =(props)=>{
    return(
        <div className="flex flex-col items-center w-fit">
            <label htmlFor={props.inputId} className="pb-1 text-sm self-start">{props.label}</label>
            <select name={props.name} multiple={props?.mutiple} defaultValue={""} className="bg-gray-300 text-sm border-[1px]"> 
                <option value={""}>-</option>
